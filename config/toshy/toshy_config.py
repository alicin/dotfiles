...
# imv image viewer. imv only listens to single-key binds (q, x, i/o, a, r, Ctrl+r,
# arrows for prev/next), so the General GUI keymap rewrites Cmd shortcuts into
# combos imv ignores -> "no Cmd shortcuts work". This maps the common macOS image
# viewer shortcuts onto imv's native keys. App keymaps run before "General GUI",
# so these win. imv's defaults live in /etc/imv_config.
hmp_is_imv                      = matchProps(clas="^imv$")
keymap("imv image viewer", {
    C("RC-q"):                  C("q"),                         # Cmd+Q  -> quit
    C("RC-w"):                  C("q"),                         # Cmd+W  -> close window (quit)
    C("RC-equal"):              C("i"),                         # Cmd+=  -> zoom in
    C("Shift-RC-equal"):        C("i"),                         # Cmd++  -> zoom in
    C("RC-minus"):              C("o"),                         # Cmd+-  -> zoom out
    C("RC-0"):                  C("a"),                         # Cmd+0  -> actual size (100%)
    C("RC-r"):                  C("C-r"),                       # Cmd+R  -> rotate 90 (imv: Ctrl+r)
}, when = lambda ctx:
    cnfg.screen_has_focus and
