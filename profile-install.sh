...
install_profile_packages() {
    ...
    case "$DETECTED_OS" in
        ...
        fedora)
            ...
            ;;
        debian|ubuntu)
            if is_package_manager_enabled "apt"; then
                local apt_file="$profile_dir/packages/$(get_package_manager_file "apt")"
                if [[ -f "$apt_file" ]]; then
                    log_step "Installing packages via apt (skipping already installed)"
                    ...
                    log_warning "APT package file not found: $apt_file"
                fi
            fi
    ...
    # npm global packages (cross-platform, runs after OS-specific package managers)
    if is_package_manager_enabled "npm"; then
        ...
        if [[ -f "$npm_file" ]]; then
            ...
            log_warning "npm package file not found: $npm_file"
        fi
    fi
}
