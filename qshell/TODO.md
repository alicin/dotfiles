...
- [x] ★ Mic privacy light keys off *unmuted* (lit all day) instead of actual
...
- [x] Panel claims to morph between menus but x/width/height snap — add
  ...
  anim (300ms *entrance* curve both ways reads sluggish on dismiss)
...
- [ ] Window search — Super+Shift+W still shells out to wofi+jq, the last
  daily surface that isn't qshell; toplevels/icons/scorer/dispatch all in-tree
- [ ] Calculator + run-command rows (math eval when the query parses; `Run:`
  as last resort instead of "No matches")
- [ ] Command palette (`>` prefix): theme set (first UI for it), capture,
  power profiles, session verbs
- [ ] Desktop-entry actions / jump lists ("New Private Window") — DesktopEntry
  exposes them, never surfaced
- [ ] Emoji picker as a `:` prefix mode over an embedded table, wl-copy on
  accept
- [ ] True MRU Alt-Tab cycling in the overview (needs an MRU stack off
  activewindow events; release-to-commit can come later)
...
*gets wrong* rather than merely lacks.
